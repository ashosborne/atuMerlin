...
- `c10` — **`closePARAMETER` would not reset the cache.** It closes the data path but leaves the buffer, so a same-key request after a close still skips the `chain`. ~~Exported by `EXPORT(*ALL)` with `chainPARAMETER`~~ **Corrected (Pack B run 16, `srvpgm-supporting-c06`): neither `closePARAMETER` nor `chainPARAMETER` carries the `export` keyword, so `EXPORT(*ALL)` exports the five getters only; `ClosePARAMETER` in the copybook has no export behind it and would fail to bind.** No binder source, no `*PRV`.
